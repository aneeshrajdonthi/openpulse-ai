        ...
        <div key={node.id} className="select-none">
          <div
            ...
            className={`flex items-center justify-between py-2 pr-3 rounded-lg text-xs cursor-pointer transition-colors ${
              isLight ? 'text-zinc-700 hover:bg-zinc-100' : 'text-zinc-300 hover:bg-zinc-800/50'
            }`}
          >
            <div className="flex items-center gap-2 truncate">
              {isFolder ? (
                <>
                  {isExpanded ? (
                    <ChevronDown className={`w-3.5 h-3.5 shrink-0 ${isLight ? 'text-zinc-400' : 'text-zinc-400'}`} />
                  ) : (
                    <ChevronRight className={`w-3.5 h-3.5 shrink-0 ${isLight ? 'text-zinc-400' : 'text-zinc-400'}`} />
                  )}
                  {isExpanded ? (
                ...
                <>
                  ...
                  <FileCode className={`w-4 h-4 shrink-0 ${isLight ? 'text-zinc-600' : 'text-zinc-300'}`} />
                </>
              )}
              <span className={`font-mono truncate ${isFolder ? `font-semibold ${isLight ? 'text-zinc-900' : 'text-white'}` : ''}`}>
                {node.name}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              {node.moduleType && (
                <span className={`px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase border ${
                  isLight ? 'bg-zinc-100 text-zinc-600 border-zinc-200' : 'bg-zinc-800 text-zinc-400 border-zinc-700'
                }`}>
                  {node.moduleType}
                </span>
              )}
              {node.fileSize && (
                <span className={`text-[10px] font-mono hidden sm:inline ${isLight ? 'text-zinc-400' : 'text-zinc-500'}`}>
                  {node.fileSize}
                </span>
              )}
            </div>
  ...
  return (
    <div className={`flex flex-col space-y-6 p-2 md:p-4 font-sans max-w-6xl mx-auto w-full transition-colors ${
      ...
      <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-5 ${
        ...
        <div>
          <div className={`flex items-center gap-2 text-xs font-mono mb-1 ${isLight ? 'text-zinc-500' : 'text-zinc-400'}`}>
            <span>Repository Architecture</span>
            <span>/</span>
            <span className={isLight ? 'text-zinc-900 font-bold' : 'text-zinc-200'}>{repo.owner}/{repo.name}</span>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={`flex items-center p-1 border rounded-lg ${
            isLight ? 'bg-zinc-100/80 border-zinc-200' : 'bg-zinc-900 border-zinc-800'
          }`}>
            <button
              onClick={() => setViewMode('graph')}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
                viewMode === 'graph'
                  ? isLight ? 'bg-white text-zinc-900 shadow-xs border border-zinc-200/80 font-bold' : 'bg-zinc-800 text-white shadow-sm font-semibold'
      ...
      {/* Main Container */}
      {viewMode === 'graph' ? (
        <div className={`relative space-y-4 pl-4 md:pl-6 border-l my-2 ${isLight ? 'border-zinc-200' : 'border-zinc-800'}`}>
            ...
            return (
              <div key={node.id} className="relative group">
                ...
                <div className={`absolute -left-[23px] md:-left-[31px] top-5 w-3.5 h-3.5 rounded-full border-2 transition-colors flex items-center justify-center ${
                  isLight 
                    ? 'bg-white border-zinc-300 group-hover:border-zinc-800' 
                    : 'bg-black border-zinc-700 group-hover:border-zinc-400'
                }`}>
                  <div className={`w-1 h-1 rounded-full ${isLight ? 'bg-zinc-800' : 'bg-zinc-400'}`} />
                </div>
                ...
                <div
                  ...
                  className={`p-4 rounded-xl border transition-all cursor-pointer ${
                    isExpanded
                      ? isLight ? 'bg-white border-zinc-300 shadow-xs' : 'bg-zinc-900/90 border-zinc-700 shadow-lg'
                      : isLight ? 'bg-white border-zinc-200/80 hover:border-zinc-300 shadow-xs' : 'bg-zinc-900/40 border-zinc-800/80 hover:border-zinc-700 hover:bg-zinc-900/60'
                  }`}
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2.5">
                        <span className={`text-sm font-bold tracking-tight ${isLight ? 'text-zinc-900' : 'text-white'}`}>
                          {node.label}
                        </span>
                        <span className={`px-2 py-0.5 rounded text-[10px] font-mono capitalize border ${
                          isLight ? 'bg-zinc-100 text-zinc-700 border-zinc-200' : 'bg-zinc-800 text-zinc-400 border-zinc-700'
                        }`}>
                          {node.type}
                        </span>
                      </div>
                    ...
                    <div className="flex items-center gap-3 shrink-0 self-end sm:self-center">
                      <span className={`text-xs font-mono px-2.5 py-1 rounded-md border ${
                        isLight ? 'bg-zinc-100/70 text-zinc-700 border-zinc-200' : 'bg-zinc-800/60 text-zinc-400 border-zinc-700/60'
                      }`}>
                        {mappedFiles.length || node.fileCount} files
                      </span>
                      <div className="w-6 h-6 rounded flex items-center justify-center text-zinc-400">
                        {isExpanded ? (
                          <ChevronDown className={`w-4 h-4 ${isLight ? 'text-zinc-900' : 'text-white'}`} />
                        ) : (
                          <ChevronRight className={`w-4 h-4 ${isLight ? 'text-zinc-400' : 'text-zinc-400'}`} />
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  {isExpanded && (
                    <div className={`mt-4 pt-4 border-t space-y-3 cursor-default ${isLight ? 'border-zinc-100' : 'border-zinc-800'}`} onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-between">
                        <span className={`text-[11px] font-mono flex items-center gap-1.5 ${isLight ? 'text-zinc-500' : 'text-zinc-400'}`}>
                          <GitCommit className={`w-3.5 h-3.5 ${isLight ? 'text-zinc-700' : 'text-zinc-300'}`} />
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1 text-xs font-semibold rounded-md transition-colors flex items-center gap-1 ${
                            isLight ? 'bg-zinc-900 hover:bg-zinc-800 text-white' : 'bg-white hover:bg-zinc-200 text-black'
                          }`}
                        >
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-start justify-between gap-2">
                                <div className="flex items-center gap-2 truncate">
                                  <FileCode className={`w-3.5 h-3.5 shrink-0 ${isLight ? 'text-zinc-700' : 'text-zinc-300'}`} />
                                  <span className={`font-mono text-xs font-bold truncate ${isLight ? 'text-zinc-900' : 'text-white'}`} title={filename}>
                                    {filename}
                                  </span>
                                </div>
                                <button
                              ...
                              <p className={`font-mono text-[10px] truncate ${isLight ? 'text-zinc-400' : 'text-zinc-500'}`} title={filePath}>
                                {filePath}
                              </p>
                              ...
                              <div className="flex items-center justify-between pt-1 text-[10px]">
                                ...
                                <span 
                                  className={`font-semibold group-hover:underline cursor-pointer flex items-center gap-0.5 ${
                                    isLight ? 'text-zinc-900' : 'text-zinc-300 hover:text-white'
                                  }`} 
        ...
        <div className={`border rounded-xl p-5 shadow-xs min-h-[450px] ${
          ...
          <div className={`flex items-center gap-2 px-3 py-2 border rounded-lg text-xs font-mono font-bold mb-3 ${
            isLight ? 'bg-zinc-100 border-zinc-200 text-zinc-900' : 'bg-black border-zinc-800 text-white'
          }`}>
            <Folder className={`w-4 h-4 ${isLight ? 'text-zinc-700' : 'text-zinc-300'}`} />
