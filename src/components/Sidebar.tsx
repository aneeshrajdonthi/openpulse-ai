...
export function Sidebar({ activeTab, onTabChange, openIssuesCount, theme = 'dark' }: SidebarProps) {
  ...
  return (
    <aside className={`w-56 h-[calc(100vh-3.5rem)] sticky top-14 flex flex-col border-r shrink-0 select-none transition-colors ${
      isLight ? 'bg-[#fafafa] border-zinc-200 text-zinc-900' : 'bg-black border-zinc-800 text-zinc-200'
    }`}>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-2.5 py-1.5 text-xs rounded-md transition-colors relative ${
                      isActive 
                        ? isLight ? 'bg-white text-zinc-900 font-bold shadow-xs border border-zinc-200' : 'bg-zinc-800/80 text-white font-semibold border border-zinc-700/60 shadow-sm'
                        : isLight ? 'text-zinc-600 hover:text-zinc-900 hover:bg-zinc-200/50' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900'
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      <Icon className={`w-4 h-4 ${isActive ? (isLight ? 'text-zinc-900' : 'text-white') : (isLight ? 'text-zinc-500' : 'text-zinc-400')}`} />
                      <span>{item.label}</span>
                    </div>
                    {item.badge !== undefined && (
                      <span className={`text-[10px] px-1.5 py-0.2 font-mono rounded ${
                        isActive 
                          ? isLight ? 'bg-zinc-900 text-white font-bold' : 'bg-zinc-700 text-white font-semibold'
      ...
      <div className={`p-3 border-t mt-auto ${isLight ? 'border-zinc-200' : 'border-zinc-800'}`}>
        <div className={`flex items-center gap-2 px-2 py-1 text-xs font-mono ${isLight ? 'text-zinc-600' : 'text-zinc-400'}`}>
